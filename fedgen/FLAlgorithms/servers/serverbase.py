...
class Server:
    def __init__(self, config, data, model, seed):
        ...
        self.num_glob_iters = self.config['num_glob_iters']
        self.local_epochs = self.config['local_epochs']
        ...
        self.deviations = {}
        self.timestamp = None
        #self.save_path = self.config['result_path']
    ...
    def select_users(self, round, num_users, return_idx=False):
        ...
        if(num_users == len(self.users)):
            ...
            user_idxs = list(range(len(self.users)))
    ...
    def save_results(self, args):
        alg = get_log_path(args, self.config['algorithm'], self.seed, self.config['gen_batch_size'])
