...
import numpy as np
from fedgen.FLAlgorithms.users.userbase import User
from torchmetrics import Accuracy
from utils.model_utils import sample_multi_label_distribution
import wandb
...
class UserpFedGen(User):
    def __init__(self,
                 ...
                 use_adam=False):
        super().__init__(config, id, model, train_loader, use_adam=use_adam)
        self.config = config
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optimizer, gamma=0.>        self.label_counts = {}
    ...
    def train(self, glob_iter, personalized=False, early_stop=True, regularization=True, verbose=False, debug=False):
        ...
        for epoch in tqdm(range(self.local_epochs)):
            ...
            if not (regularization and epoch < early_stop):
