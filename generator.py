...
def create_generative_model(dataset, algorithm='', model='cnn', embedding=False, device=torch.device('cpu')):
    ...
    if 'FedGen' in algorithm:
        ...
        if 'cnn' in algorithm:
            ...
            passed_dataset += '-cnn1'
    return Generator(dataset=passed_dataset, model=model, embedding=embedding, latent_layer_idx=-1, device=device)

class Generator(nn.Module):
    def __init__(self, dataset='mnist', model='cnn', embedding=False, latent_layer_idx=-1, device=torch.device('cpu')):
        super(Generator, self).__init__()
        self.embedding = embedding
        self.dataset = dataset
        self.latent_layer_idx = latent_layer_idx
        self.hidden_dim, self.latent_dim, self.input_channel, self.n_class, self.noise_dim = (512, 2048 ,None ,19, 32)
    ...
    def build_network(self):
        """
        module_list = []
        module_list.append(self.Linear(self.fc_configs[0], self.fc_configs[1]))
        module_list.append(nn.BatchNorm1d(out_dim))
        module_list.append(nn.ReLU())
        module_list.append(nn.Linear(self.fc_configs[-1], self.latent_dim))
        self.model = nn.ModuleList(module_list)
        self.model.to(self.device)
        """
        if self.embedding:
            self.embedding_layer = nn.Embedding(self.n_class, self.noise_dim).to(self.device)
        ### FC modules ####
        self.fc_layers = nn.ModuleList()
        for i in range(len(self.fc_configs) - 1):
            input_dim, out_dim = self.fc_configs[i], self.fc_configs[i + 1]
            fc = nn.Linear(input_dim, out_dim).to(self.device)
            bn = nn.BatchNorm1d(out_dim).to(self.device)
            act = nn.ReLU().to(self.device)
            self.fc_layers += [fc, bn, act]
        ### Representation layer
        self.representation_layer = nn.Linear(self.fc_configs[-1], self.latent_dim).to(self.device)

    def forward(self, y_input, latent_layer_idx=-1, verbose=True):
        """
        G(Z|y) or G(X|y):
