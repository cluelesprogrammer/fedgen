...
class SmallEarthNet(VisionDataset):
    """BigEarthNet dataset.
    ...
    * https://doi.org/10.1109/IGARSS.2019.8900532
    ...
    def __init__(
        ...
        if country is not None:
            self.splits_metadata["train"]["filename"] = country + "-train.csv"
            self.metadata["s2"]["directory"] = country
        # self._verify()
    ...
    def _load_paths(self, index: int) -> List[str]:
        ...
        if self.bands == "all":
            ...
            folder = Path(self.root) / self.folders[index]["s2"]
            paths = glob.glob(os.path.join(folder, "*.tif"))
    ...
    def get_folders(self) -> List[str]:
        ...
        folders = [os.path.join(self.root, dir_s2, pair[0]) for pair in pairs]
        return folders
    ...
    def _load_image(self, index: int) -> Tensor:
        ...
        for path in paths:
            ...
            with rasterio.open(path) as dataset:
                ...
                images.append(array)
        arrays: "np.typing.NDArray[float_]" = np.stack(images, axis=0)
    ...
    def _load_target(self, index: int) -> Tensor:
        ...
        if self.num_classes == 19:
            indices_optional = [
                self.label_converter.get(idx) for idx in indices
            ]
    ...
    def _verify(self) -> None:
        ...
        urls.extend(
            [self.splits_metadata[k]["url"] for k in self.splits_metadata]
        )
        md5s.extend(
            [self.splits_metadata[k]["md5"] for k in self.splits_metadata]
        )
        filenames_splits = [
